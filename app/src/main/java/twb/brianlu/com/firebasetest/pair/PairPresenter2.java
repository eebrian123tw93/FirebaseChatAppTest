...
//                            setRippleViewUnPairMode();
        ...
        if (!isPairing) {
            Observer<Boolean> matchResult = new Observer<Boolean>() {
                @Override
                public void onSubscribe(Disposable d) {
            ...
            PairApiService.getInstance()
                    .pair(observer, FirebaseAuth.getInstance().getCurrentUser().getUid(), false);
        } else {
            PairApiService.getInstance()
                    .cancelPair(observer, FirebaseAuth.getInstance().getCurrentUser().getUid(), false);
            setRippleViewUnPairMode();
        }
    }
